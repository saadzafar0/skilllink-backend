...
router.post("/", async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: "Skill added successfully", skillID: result.recordset[0].skillID });
    } catch (error) {
...
router.put("/:skillID", async (req, res) => {
    ...
    try {
        ...
        const result = await pool.request()
            .input("skillID", sql.Int, skillID)
            .input("skillName", sql.VarChar(255), skillName)
            .query("UPDATE Skills SET skillName = @skillName WHERE skillID = @skillID");
